 ...
 */
@Path("/radik") public class RegistrationRest {

    @GET @Path("/whoami") public Response getWhoAmIByInn() {
        return Response.status(301).header(HttpHeaders.LOCATION, "/radik/whoami.jsp").build();
    }
    ...
    @GET @Path("/list") public Response getList() {
        return Response.status(301).header(HttpHeaders.LOCATION, "/list.jsp").build();
    }

    @POST @Path("/regme") public Response upload(@FormParam("invite") String invite, @FormParam("key") String pubKey) {
    ...
    @GET @Path("/whoismine") public Response getRes() {
        return Response.status(301).header(HttpHeaders.LOCATION, "/whoismine.jsp").build();
    }

    @POST @Path("/whoismine") public Response whoIsMine(@FormParam("key") String prKey) {
    ...
    @GET @Path("/whoami") public Response getWhoAmI() {
        return Response.status(301).header(HttpHeaders.LOCATION, "/whoami.jsp").build();
    }

    @POST @Path("/whoami") public Response whoAmI(@FormParam("invite") String invite) {
        StringBuilder res = new StringBuilder();
        try {
            Session session = DBSession.getSession();
    ...
    @GET @Path("/shuffle") public Response getShuffler() {
        return Response.status(301).header(HttpHeaders.LOCATION, "/shuffler.jsp").build();
    }

    @POST @Path("/shuffle") public Response shuffle(@FormParam("pass") String pass) {
